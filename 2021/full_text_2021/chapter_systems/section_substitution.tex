...
\begin{document}
\section{The Substitution Method (L\arabic{lesson_substitution})}
...
%    x = 6 - 5~~~~~~~ &  & \tmop{Next~subtract}\\
  ...
  \end{eqnarray*}
\end{example}
The process in the previous example is known as solving by substitution. This process is described and illustrated in the following table which lists the five steps to solving by substitution.
\begin{center}
\begin{tabular}{|l|c|}
  \hline
...
%      \text{Find the lone variable:~}x \text{~in the first or second equation,}\\
%			\text{or~} y \text{~in the first equation.}\\ \text{We will choose~} x \text{~in the first equation}.
%    \end{array}\\
